...
| `layout` | 置き場所と役割。ここに無い場所への新規文書は差し戻す。**実在するものだけ書く** |
| `limits` | パスごとの行数上限。超えたら指摘する |
| `claude_md` | CLAUDE.md の**節ごと**の上限。全体の行数だけでは足りない |
| `overlap` | 同じ見出しが複数の文書に出ていないか。**散らばりを見る** |
| `duplication` | 同じ文が複数の文書に書かれていないか。**二重管理を見る** |
| `history` | 正典に開発の履歴が混ざっていないか。**溜まりかたを見る** |
| `watch` | 「ここを触ったらここを直す」の対応表 |
| `archive` | 記録の置き場・索引・分け方 |
| `guard` | 置き場所チェックの範囲と拡張子 |

**`layout` に実在しない場所を書かない。**雛形の `layout` は候補の一覧で、
そのまま使う規約ではない。実在しない行を残すと `guard` はそこへの書き込みを
許し、`watch` はそこが未更新だと言い続ける — **どちらも直しようのない
嘘の指摘になる。**`check` の「規約にあるが実在しない置き場所」が拾う。
...
### 散らばりと二重管理を見る (claude_md / overlap / duplication)

`limits` `layout` `index` はどれも**ファイル 1 つを見る**検査で、
「置き場所は合っている・行数も上限内・索引からも辿れる。ただし仕様が
3 つの文書に散っている」を通してしまう。この 3 つがその穴を塞ぐ。

```yaml
  claude_md:
    path: CLAUDE.md
    max_section_lines: 25          # これを超える節は docs へ出す候補

  overlap:                         # 見出しの重なり = 話題が散っている
    enabled: true
    scope: ["*.md", "docs/*.md"]
    ignore: []
    min_docs: 2

  duplication:                     # 字面の重なり = 同じ事実を 2 か所で管理している
    enabled: true
    min_lines: 3
    min_chars: 80
    scope: ["*.md", "docs/**"]
...
| 検査 | 見えるもの | 見えないもの |
|---|---|---|
| `claude_md` | CLAUDE.md の節が設計書に育っている | 全体の行数が上限内かどうかとは無関係 |
| `overlap` | 見出しが同じ = 話題が散っている | 見出しが違えば同じ話でも出ない |
| `duplication` | 字面がそろった写し | **言い換えて写したもの** |

- **3 つとも出すのは場所だけ。**同じ事実かどうかは、
  「変わるときに一緒に変わるか」で決まる(skill `single-source`)。
  決めるのは `/docs` と役 `duplication-auditor`
- **`duplication` が 0 件でも二重管理が無いとは言えない。**言い換えて写した
  ものはここに出ない。役に読ませないと見つからない
- `ignore` に足すのは「重なっていて正しい」と**確かめたもの**だけ。
  鳴るからという理由で足すと、次に本当に散ったときに黙る

### 正典に混ざる履歴を見る (history)

`overlap` と `duplication` が**横に散る**のを見るのに対して、`history` は
**時間で溜まる**のを見る。`/docs` の既定は「実装に合わせる」= 追記なので、
変更のたびに「いつ何をこう変えた」が仕様書へ流れ込む。**1 回ごとは 2 行なので
誰も止めない。**溜まると、いまの姿を読むのに履歴をかき分けることになる。

```yaml
  history:
    enabled: true
    scope: ["*.md", "docs/**"]     # 記録 (archive) は自動で外れる
    allow:                         # 履歴を書くのが仕事の文書
      - docs/changelog.md
```

**`limits` では捕まらない。**履歴混じりの 180 行は上限 200 行を通る。

- **拾えるのは日付と決まった言い回しだけ** (「以前は」「〜に変更した」
  「v1.2 から」)。言い換えられた経緯は出ないので、役 `docs-auditor` が読む
- `allow` に足すのは**履歴を書くのが役割の文書**だけ。研究なら試行記録と
  結果表、製品なら `changelog` と出荷手順。**鳴るからという理由で足すと、
  その文書は二度と見られなくなる**
- 剥がし方は `/docs` の「正典から履歴を剥がす」。**消すのではなく、
  現在形に直すか記録へ移す**
